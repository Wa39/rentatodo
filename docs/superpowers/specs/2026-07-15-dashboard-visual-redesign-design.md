...
Plus new custom (non-shadcn-reserved) CSS variables/Tailwind color-extension keys for tones shadcn's default slots don't cover, needed for pending/active states and the sidebar:
...
--sidebar-bg: 152 23% 11%;         /* sidebar */
--sidebar-hover: 153 23% 16%;
--sidebar-foreground: 138 12% 83%; /* base sidebar text */
```

`apps/web/tailwind.config.ts`'s `theme.extend.colors` gains `warning`/`warning-foreground`/`warning-tint`, `info`/`info-foreground`/`info-tint`, `sidebar-bg`/`sidebar-hover`/`sidebar-foreground` (all `hsl(var(--x))`-wrapped, same pattern as the existing `primary`/`destructive`/etc.). Lighter sidebar text shades used ad hoc in the mockup (`#b7c4bd` nav-item text, `#8a988f` footer-role text, `#6b7a72` section-label text) are approximated via Tailwind opacity modifiers on `sidebar-foreground` (e.g. `text-sidebar-foreground/80`) rather than four more named tokens — YAGNI, avoids one-off token proliferation for text-opacity variance.
