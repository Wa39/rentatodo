...
**Week:** 1 (Auth + Items) merged and done — Week 2 (Reservations, the
contract's "vertical slice") is next.
**Last updated:** 2026-07-16
**Current focus:** All planned Week 1 work is merged to `develop`. `User`
+ Auth (PR #8), CORS + validation handler (PR #14), and `Item` model +
the 3 Days 4-5 endpoints (`POST /items`, `GET /items` filtered/searchable,
`GET /items/{item_id}`, PR #16) are all in. `PATCH`/`DELETE /items/{id}`/
`GET /users/me/items` deliberately deferred to a follow-up (confirmed
with Jose).
...
- Nothing in `apps/api` right now — Week 1 is fully merged. Next step is starting Reservations design (see below).
